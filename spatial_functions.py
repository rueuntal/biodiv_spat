...
def gen_sp_range_contiguous(Ncell, sp_cont_cells, wrapped = True, ymax = None):
    ...
    while len(sp_range) < Ncell:
        ...
        except:  # when the neighbours are exhausted, jump to a new location (e.g., island)
...
def corr_null_range_contiguous(sp_range_array, sp_list_array, continent, out_dir, cont_geom = None, Niter = 200):
    """Randomly distribute species on the landscape, keeping the range size of each species, and compute correlation
    
    between overall S and partial S on these simulated landscapes.
    Species ranges are contiguous and generated by a spreading dye algorithm.
    ...
    if continent is not 'global':
        ...
        sp_cont_cells = [(i, j) for j in range(len(sp_list_array[0])) for i in range(len(sp_list_array)) \
                    if (cont_array[i][j] == 1 and len(sp_list_array[i][j]) > 0)]
    else: sp_cont_cells = [(i, j) for j in range(len(sp_list_array[0])) for i in range(len(sp_list_array)) if len(sp_list_array[i][j]) > 0]
    ...
    for i in range(Niter):
        sp_list_sim = [[] for grid in  sp_cont_cells]
        for sp in sp_Ncell.keys():
            n_sp = sp_Ncell[sp]
